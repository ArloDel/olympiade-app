...
import { Activity, Clock, ShieldCheck, Sun, Moon, LogOut, Menu, X, FileText, CheckCircle, Database, Award } from "lucide-react"
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    } else if (status === "authenticated" && !["ADMIN", "SUPERADMIN"].includes((session?.user as any)?.role)) {
  ...
  const navLinks = [
    { name: "Monitoring", href: "/admin", icon: <Activity size={18} /> },
    { name: "Manajemen Ujian", href: "/admin/exams", icon: <Clock size={18} /> },
    { name: "Manajemen Soal", href: "/admin/questions", icon: <FileText size={18} /> },
    { name: "Bank Soal", href: "/admin/question-bank", icon: <Database size={18} /> },
    { name: "Koreksi Esai", href: "/admin/grading", icon: <CheckCircle size={18} /> },
    { name: "Rekap Nilai", href: "/admin/results", icon: <Award size={18} /> },
  ]
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                <div className={`${isActive ? (isDark ? 'text-black dark:text-white' : 'text-black') : ''}`}>
                  {link.icon}
                </div>
                {link.name}
              </Link>
